...
const Board = ({ columns }) => {
  ...
  columns.forEach(
    (column) =>
      (stories_by_column[column] = stories.filter(
        (item) => item.estado === column
      ))
  );
  ...
  const onDragEnd = (result) => {
    ...
    if (source === dest) {
      const newList = Array.from(
        stories_by_column[source].sort((a, b) => a.index - b.index)
      );
      ...
      const newListSource = Array.from(
        stories_by_column[source].sort((a, b) => a.index - b.index)
      );
      const newListDest = Array.from(
        stories_by_column[dest].sort((a, b) => a.index - b.index)
      );
        ...
        {columns.map((column) => {
          return (
            <div className="column" key={column}>
              <h2 className="is-size-4 has-text-weight-medium">{column}</h2>
              <StateDroppable
                stateItems={stories_by_column[column]}
                id={column}
              />
            </div>
          );
